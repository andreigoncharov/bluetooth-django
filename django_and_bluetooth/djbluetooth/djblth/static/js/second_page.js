...
function clearGraph(){
    ...
    $.ajax({
        ...
        error: function (response) {
            console.log('Error in clearGraph request');
        }
    });
}
...
async function setCurrentCycle(){
    $.ajax({
        url: `${setCurrentCycleUrl}`,
        method: 'POST',
        data: JSON.stringify({user_id: currentUserId, cycle_number:currentCycle}),
        success: function (response) {
        },
        error: function (response) {
            console.log('Error in setCurrentCycle request');
        }
    });
}